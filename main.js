import { app, Tray, Menu, nativeImage, BrowserWindow } from "electron/main";
...
const createWindow = () => {
  ...
  const contextMenu = Menu.buildFromTemplate([
    { label: "Settings", type: "normal", click: openPreferences },
    { label: "On", type: "radio" },
    { label: "Off", type: "radio" },
    {
      label: isConnected ? "Disconnect" : "Connect",
      type: "normal",
      click: arduinoConnection,
      id: "Connection",
    },
  ]);
...
async function arduinoConnection() {
  ...
  const [arduinoPort] = ports.filter(
    (x) => x.vendorId == "2341" || x.productId == "0043"
  );
...
function openPreferences(menuItem) {
...
app.whenReady().then(() => {
  ...
  socket.on("connect", () => {
    if (!store.has("can_slap")) {
      store.set("can_slap", false);
    }
    if (store.has("settings")) {
      const settings = store.get("settings");
      socket.emit("label-socket", settings.teamChannel);
    }
    ...
    socket.on("slap", (event) => {
      const settings = store.get("settings");
      if (event.text.indexOf(settings.profileName) > -1) {
        if (port) {
          port.write("SLAP|");
        }
      }
